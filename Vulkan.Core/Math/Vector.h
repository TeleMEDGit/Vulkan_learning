...
namespace Math
{
	...
	public:
		...
		INLINE float GetLength() {
			...
			auto length = XMVector3Length(m_vec);			

			return  XMVectorGetX(m_vec);
		}
		INLINE bool Compare(const Vector3 &a, const float epsilon) const {
			if (Fabs(XMVectorGetX(m_vec) - a.GetX()) > epsilon) {
				return false;
			}

			if (Fabs(XMVectorGetY(m_vec) - a.GetY()) > epsilon) {
				return false;
			}

			if (Fabs(XMVectorGetZ(m_vec) - a.GetZ()) > epsilon) {
				return false;
			}
		...
		INLINE bool FixDegenerateNormal()
		{
