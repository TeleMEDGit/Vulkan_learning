...
namespace Math
{
	...
	public:
		...
		{
			m_mat.c[0] = SetWToZero(x); m_mat.c[1] = SetWToZero(y);
			m_mat.c[2] = SetWToZero(z); m_mat.c[3] = SetWToOne(w);
		}
		INLINE Matrix4(Vector4 x, Vector4 y, Vector4 z, Vector4 w) { m_mat.c[0] = x; m_mat.c[1] = y; m_mat.c[2] = z; m_mat.c[3] = w; }
		INLINE Matrix4(const Matrix4& mat) { m_mat = mat.m_mat; }
		INLINE Matrix4(const Matrix3& mat)
		{
			m_mat.c[0] = SetWToZero(mat.GetX());
			m_mat.c[1] = SetWToZero(mat.GetY());
			m_mat.c[2] = SetWToZero(mat.GetZ());
			m_mat.c[3] = CreateWUnitVector();
		}
		INLINE Matrix4(const Matrix3& xyz, Vector3 w)
		{
			m_mat.c[0] = SetWToZero(xyz.GetX());
			m_mat.c[1] = SetWToZero(xyz.GetY());
			m_mat.c[2] = SetWToZero(xyz.GetZ());
			m_mat.c[3] = SetWToOne(w);
		}
		...
		INLINE explicit Matrix4(EZeroTag) { m_mat.c[0] = m_mat.c[1] = m_mat.c[2] = m_mat.c[3] = SplatZero(); }

		INLINE const Matrix3& Get3x3() const { return (const Matrix3&)*this; }

		INLINE Vector4 GetX() const { return Vector4(m_mat.c[0]); }
		INLINE Vector4 GetY() const { return Vector4(m_mat.c[1]); }
		INLINE Vector4 GetZ() const { return Vector4(m_mat.c[2]); }
		INLINE Vector4 GetW() const { return Vector4(m_mat.c[3]); }

		INLINE void SetX(Vector4 x) { m_mat.c[0] = x; }
		INLINE void SetY(Vector4 y) { m_mat.c[1] = y; }
		INLINE void SetZ(Vector4 z) { m_mat.c[2] = z; }
		INLINE void SetW(Vector4 w) { m_mat.c[3] = w; }
