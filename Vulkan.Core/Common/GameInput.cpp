...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel(AppCore::g_hWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel(AppCore::g_hWnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground !=AppCore::g_hWnd // wouldn't be able to acquire
            || !visible)
        {
            KbmZeroInputs();
        }
