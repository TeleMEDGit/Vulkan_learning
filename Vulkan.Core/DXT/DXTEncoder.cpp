...
========================
*/
inline void idDxtEncoder::ExtractBlock( const byte *inPtr, int width, byte *colorBlock ) const {
	for ( int j = 0; j < 4; j++ ) {
...
int NormalDistanceDXT1( const int *vector, const int *normalized ) {
	...
	float rcplen = Math::InvSqrt( floatNormal[0] * floatNormal[0] + floatNormal[1] * floatNormal[1] + floatNormal[2] * floatNormal[2] );
	floatNormal[0] *= rcplen;
	floatNormal[1] *= rcplen;
	floatNormal[2] *= rcplen;
	intNormal[0] = Math::Ftob( ( floatNormal[0] + 1.0f ) * ( 255.0f / 2.0f ) + 0.5f );
	intNormal[1] = Math::Ftob( ( floatNormal[1] + 1.0f ) * ( 255.0f / 2.0f ) + 0.5f );
	intNormal[2] = Math::Ftob( ( floatNormal[2] + 1.0f ) * ( 255.0f / 2.0f ) + 0.5f );
...
int NormalDistanceDXT5( const int *vector, const int *normalized ) {
	...
	floatNormal[1] *= rcplen;
	floatNormal[2] *= rcplen;
	intNormal[c0] = Math::Ftob( ( floatNormal[0] + 1.0f ) / 2.0f * 255.0f + 0.5f );
	intNormal[c1] = Math::Ftob( ( floatNormal[1] + 1.0f ) / 2.0f * 255.0f + 0.5f );
	intNormal[c2] = Math::Ftob( ( floatNormal[2] + 1.0f ) / 2.0f * 255.0f + 0.5f );
...
void idDxtEncoder::BiasScaleNormalY( byte *colorBlock ) const {
	...
	int bestBias = 128;
	int bestRange = std::max( abs( minColor - bestBias ), abs( maxColor - bestBias ) );
#if USE_BIAS
	for ( int i = 0; i < 32; i++ ) {
		int bias = ( ( i << 3 ) | ( i >> 2 ) ) - 4;
		int range = std::max( abs( minColor - bias ), abs( maxColor - bias ) );
		if ( range < bestRange ) {
			bestRange = range;
			bestBias = bias;
		}
	...
	if ( scale == 1 ) {
		...
		if ( bestBias != 128 ) {
			c_differentBias++;
			int r = std::max( abs( minColor - 128 ), abs( maxColor - 128 ) );
			int s = 1 + ( r <= s0 ) + 2 * ( r <= s1 );
			if ( scale > s ) {
				c_biasHelped++;
			}
...
void idDxtEncoder::RotateNormalsDXT5( byte *block ) const {
	...
	for ( int i = 0; i < 32; i += 1 ) {
		...
		int range = std::max( abs( minColor - 128 ), abs( maxColor - 128 ) );
...
========================
*/
inline void idDxtEncoder::GetMinMaxBBox( const byte *colorBlock, byte *minColor, byte *maxColor ) const {
...
========================
*/
inline void idDxtEncoder::InsetColorsBBox( byte *minColor, byte *maxColor ) const {
	byte inset[4];
...
========================
*/
inline void idDxtEncoder::InsetYCoCgAlpaBBox( byte *minColor, byte *maxColor ) const {
	int inset[4];
	int mini[4];
	int maxi[4];
...
void idDxtEncoder::ConvertNormalMapDXN2_DXT5( const byte *inBuf, byte *outBuf, int width, int height ) {
	...
	for ( int j = 0; j < height; j += 4 ) {
		for ( int i = 0; i < width; i += 4, inBuf += 16, outBuf += 16 ) {
			...
			byte maxNormalY = 0;
			for ( int k = 0; k < 16; k++ ) {
				if ( values[k] < minNormalY ) {
					minNormalY = values[k];
				}
				if ( values[k] > maxNormalY ) {
					maxNormalY = values[k];
				}
			}
